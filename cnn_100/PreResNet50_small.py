...
class PreActResNetSmall(nn.Module): # 重命名类以清晰区分
    ...
    # *** 修改: 默认类别数改为 100 ***
    def __init__(self, block, num_blocks, num_classes=100, base_planes=16): # <--- 修改默认值
        ...
        self.linear = nn.Linear(final_planes, num_classes) # <--- 使用传入的 num_classes
...
# *** 修改: 默认类别数改为 100 ***
def PreActResNet50(num_classes=100): # <--- 修改默认值
    """构建一个用于 CIFAR-100 的具有减少通道数的 PreActResNet-50 模型。"""
...
if __name__ == '__main__':
    ...
    print("正在运行 PreActResNet50_small.py 进行 CIFAR-100 训练...") # <-- 修改
    set_seed(42)

    # --- 配置参数 ---
    # *** 修改: 更新模型名称 ***
    MODEL_NAME = "PreActResNet50Small_CIFAR100" # <--- 修改
    ...
    # GUI 兼容名称（如果需要，可以不改，保持与原 GUI 配置一致）
    GUI_COMPATIBLE_NAME = 'preactresnet50_small_cifar100.pth' # <--- (可选) 修改 GUI 兼容名称
    ...
    # *** 修改: 调用工厂函数，该函数创建的是 small 版本，并传递 num_classes=100 ***
    model = PreActResNet50(num_classes=100).to(device) # <--- 修改
