...
# --- 模型定义 (PreActResNet Basic Block 和 Model) ---
class PreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.relu1 = nn.ReLU(inplace=True) # 使用 inplace ReLU
        ...
        # 如果 stride 不为 1 或通道数不同，则匹配 shortcut 维度
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes, kernel_size=1, stride=stride, bias=False)
                # 注意：原始 PreActResNet 论文在 basic block 的 shortcut 中不使用 BN
            )

    def forward(self, x):
        out = self.relu1(self.bn1(x))
        shortcut = self.shortcut(out) # 将 shortcut 应用于预激活的输入
        out = self.conv1(out)
...
class PreActResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=100): # <--- 修改默认值
        ...
        # 适应 CIFAR-10/100 的初始卷积层
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        # 注意：原始 PreActResNet 在第一个 conv 之后、主块之前应用 BN 和 ReLU。
        # 这里遵循一种常见的实现结构。
    ...
    def forward(self, x):
        out = self.conv1(x)
        # 注意：在 layer1 之前的 BN/ReLU 有时会在此处添加
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
...
# *** 修改: 默认类别数改为 100 ***
def PreActResNet18(num_classes=100): # <--- 修改默认值
    """构建一个用于 CIFAR-100 的 PreActResNet-18 模型。"""
...
if __name__ == '__main__':
    ...
    print("正在运行 PreActResNet18.py 进行 CIFAR-100 训练...") # <-- 修改
    set_seed(42)

    # --- 配置参数 ---
    # *** 修改: 更新模型名称 ***
    MODEL_NAME = "PreActResNet18_CIFAR100" # <--- 修改
    ...
    model = PreActResNet18(num_classes=100).to(device) # <--- 修改
