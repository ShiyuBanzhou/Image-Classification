...
# --- 模型定义 ---
class VGG16CIFAR10(nn.Module): # 类名保持不变，但现在用于 CIFAR-100
    """为 CIFAR-100 调整的 VGG16 模型。"""
    # *** 修改: 默认类别数改为 100 ***
    def __init__(self, num_classes=100, dropout_prob=0.5, pretrained=False): # <--- 修改默认值
        ...
        # CIFAR-100 图像是 32x32。经过 VGG16 特征层（5个最大池化）后，尺寸变为 1x1
        # VGG16 特征层输出 512 个通道。
...
if __name__ == '__main__':
    ...
    print("正在运行 VGG16.py 进行 CIFAR-100 训练...") # <-- 修改
    set_seed(42)

    # --- 配置参数 ---
    # *** 修改: 更新模型名称 ***
    MODEL_NAME = "VGG16_CIFAR100" # <--- 修改
    ...
    model = VGG16CIFAR10(num_classes=100, pretrained=USE_PRETRAINED).to(device) # <--- 修改
