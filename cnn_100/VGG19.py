...
# --- 模型定义 ---
class VGG19CIFAR10(nn.Module): # 类名保持不变，但现在用于 CIFAR-100
    """为 CIFAR-100 调整的 VGG19 模型。"""
    # *** 修改: 默认类别数改为 100 ***
    def __init__(self, num_classes=100, dropout_prob=0.5, pretrained=False): # <--- 修改默认值
        super(VGG19CIFAR10, self).__init__()
        # 加载 VGG19 特征提取器（可选预训练权重）
        vgg19_features = models.vgg19(weights=models.VGG19_Weights.IMAGENET1K_V1 if pretrained else None).features

        self.features = vgg19_features
        # CIFAR-100 图像是 32x32。经过 VGG19 特征层（5个最大池化）后，尺寸变为 1x1
        # VGG19 特征层输出 512 个通道。
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(4096, num_classes), # <--- 使用传入的 num_classes
        )
        if not pretrained:
...
if __name__ == '__main__':
    ...
    print("正在运行 VGG19.py 进行 CIFAR-100 训练...") # <-- 修改
    set_seed(42)

    # --- 配置参数 ---
    # *** 修改: 更新模型名称 ***
    MODEL_NAME = "VGG19_CIFAR100" # <--- 修改
    ...
    model = VGG19CIFAR10(num_classes=100, pretrained=USE_PRETRAINED).to(device) # <--- 修改
