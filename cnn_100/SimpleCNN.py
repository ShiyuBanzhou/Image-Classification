...
class SimpleCNN(nn.Module):
    """一个用于 CIFAR-100 的简单 CNN 模型。"""
    # *** 修改: 默认类别数改为 100 ***
    def __init__(self, num_classes=100): # <--- 修改默认值
        ...
        self.fc2 = nn.Linear(512, num_classes) # <--- 使用传入的 num_classes
...
if __name__ == '__main__':
    ...
    print("正在运行 SimpleCNN.py 进行 CIFAR-100 训练...") # <-- 修改
    set_seed(42) # 设置随机种子

    # --- 配置参数 ---
    # *** 修改: 更新模型名称 ***
    MODEL_NAME = "SimpleCNN_CIFAR100" # <--- 修改
    NUM_EPOCHS = 100     # CIFAR-100 可能需要更多 Epochs
    BATCH_SIZE = 128
    LEARNING_RATE = 0.001
    WEIGHT_DECAY = 1e-4
    SAVE_INTERVAL = 10
    ...
    try:
        ...
        print(f"下载或加载 CIFAR-100 数据集时出错: {e}") # <-- 修改
        exit()
    ...
    model = SimpleCNN(num_classes=100).to(device) # <--- 修改
